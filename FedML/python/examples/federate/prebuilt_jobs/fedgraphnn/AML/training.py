...
def train_homo(tr_loader, val_loader, te_loader, tr_inds, val_inds, te_inds, model, optimizer, loss_fn, args, config, device, val_data, te_data):
    ...
    for epoch in range(config.epochs):
        ...
        #evaluate
        val_f1, val_roc, val_recall, val_precision = evaluate_homo(val_loader, val_inds, model, val_data, device, args)
        te_f1, te_roc, te_recall, te_precision = evaluate_homo(te_loader, te_inds, model, te_data, device, args)

        if args.wandb == True:
            wandb.log({"f1/validation": val_f1}, step=epoch)
            wandb.log({"f1/test": te_f1}, step=epoch)
            wandb.log({"roc/test": te_roc}, step=epoch)
            wandb.log({"recall/test": te_recall}, step=epoch)
            wandb.log({"precision/test": te_precision}, step=epoch)
        logging.info(f'Validation F1: {val_f1:.4f}, ROC-AUC: {val_roc:.4f}, Recall: {val_recall:.4f}, Precision: {val_precision:.4f}')
        logging.info(f'Test F1: {te_f1:.4f}, ROC-AUC: {te_roc:.4f}, Recall: {te_recall:.4f}, Precision: {te_precision:.4f}')
